...
class TestCreateRecruiter:
    def test_returns_recruiter_object(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
        assert rec is not None

    def test_recruiter_id_assigned(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
    ...
    def test_fields_stored_correctly(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe", "HR Manager"
        )
        assert rec.first_name == "Jane"
        assert rec.last_name == "Doe"
        assert rec.job_title == "HR Manager"

    def test_email_stored_correctly(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
    ...
    def test_job_title_defaults_null(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# get_recruiter / get_recruiter_by_email
# ─────────────────────────────────────────────────────────────────────────────


class TestGetRecruiter:
    def test_returns_correct_recruiter_by_pk(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
    ...
    def test_get_by_email_returns_correct_recruiter(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
        fetched = get_recruiter_by_email(session, "jane@test.com")
        assert fetched.recruiter_id == rec.recruiter_id

    def test_get_by_email_returns_none_for_missing(self, session):
        assert get_recruiter_by_email(session, "nobody@test.com") is None

    def test_fields_match_after_fetch(self, session, company):
        rec = create_recruiter(
            session, "alice@test.com", company.company_id, "Alice", "Smith", "Recruiter"
        )
        fetched = get_recruiter(session, rec.recruiter_id)
...
class TestUpdateRecruiter:
    def test_update_first_name(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
    ...
    def test_update_last_name(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
        rec.last_name = "Smith"
    ...
    def test_update_job_title(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
    ...
    def test_other_fields_unaffected(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe", "HR"
        )
        rec.first_name = "Janet"
        update_recruiter(session, rec)
        fetched = get_recruiter(session, rec.recruiter_id)
...
class TestDeleteRecruiter:
    def test_delete_returns_true(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
    ...
    def test_recruiter_not_found_after_delete(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
    ...
    def test_get_by_email_returns_none_after_delete(self, session, company):
        rec = create_recruiter(
            session, "jane@test.com", company.company_id, "Jane", "Doe"
        )
        delete_recruiter(session, rec.recruiter_id)
        assert get_recruiter_by_email(session, "jane@test.com") is None
